...
inline vec3 random_in_unit_disk() { // for dof disk 
    while (true) {
...
inline vec3 reflect(const vec3& v, const vec3& n) { // reflected ray = v + 2b where b is proj of v onto n
    return v - 2*dot(v,n)*n;
}

inline vec3 refract(const vec3& uv, const vec3& n, double etai_over_etat) { // using snell's law of refratcion
